...
### Phase 1: Scaffold
**Goal**: A working Vite + React 19 + TypeScript (strict) project exists with Tailwind CSS, pnpm, oxlint, and oxfmt all configured and enforced
...
**Success Criteria** (what must be TRUE):
  1. CHOSEONG_INDEX, JUNGSEONG_INDEX, JONGSEONG_INDEX, ROTATION_SETS, and COMBINATION_RULES are exported from jamo-data.ts and use only U+3130–U+318F codepoints
  2. getNextRotation() cycles ㄱ→ㄴ→ㄱ and getRotationOptions() returns [] for non-rotatable jamo
...
Note: Phase 7 depends only on Phase 4, so it can run in parallel with Phases 5–6 in
practice — but the default execution order is sequential.
...
| 6. Game State | 0/? | Not started | - |
| 7. Word Data | 0/? | Not started | - |
| 8. Core UI | 0/? | Not started | - |
| 9. Screens and Shell | 0/? | Not started | - |

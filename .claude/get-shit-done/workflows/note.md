...
- **Global scope**: `/workspaces/binglebingle/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/workspaces/binglebingle/.claude/notes/`)
...
2. Glob `/workspaces/binglebingle/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/workspaces/binglebingle/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/workspaces/binglebingle/.claude/notes/` — works in any directory

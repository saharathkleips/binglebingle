...
- `decompose(char)` — steps a `Character` back by one construction level, splitting compound batchim jongseong into its two constituent consonant `Character` objects. Returns an array of simpler `Character` objects (may be empty). Note: `decompose` is the inverse of `compose` — not of `composeSyllable`. Each call steps back one construction level (e.g. removes jongseong, or splits compound batchim), while `composeSyllable` is a low-level Unicode arithmetic utility.

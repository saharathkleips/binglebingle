...
- `NavBar({ onToggleInstructions, isInstructionsOpen })` — renders the `ㅂㄱㅂㄱ` logo, three
  ...
  `isInstructionsOpen`
- `NavActionButton({ ariaLabel, children, testId, isExpanded, onClick })` — shared circular
  neo-brutalist nav action with the submission button surface depth, press animation, and label
  treatment

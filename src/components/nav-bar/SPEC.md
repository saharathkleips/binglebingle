...
- Nav controls keep a 44px minimum button footprint across compact width/height breakpoints for touch
  accessibility. Compact tiers tighten only the inner circular faces, label/icon sizes, logo, and gaps so
  the bar fits cramped screens without shrinking the interactive hitbox. The CSS stores tier dimensions
  in local custom properties so compact breakpoints override sizing decisions without repeating width,
  height, and icon declarations.
